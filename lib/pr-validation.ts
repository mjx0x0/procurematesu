...
export function validatePurchaseRequest(input: {
  ...
  else if (looksGibberish(purpose)) errors.purpose = 'The purpose appears invalid or contains random text. Describe what the procurement is for.';
  if (department.length < 2 || looksGibberish(department)) errors.department = 'Please enter a valid department or university unit.';
  ...
  if (!input.items.length) errors.items = 'Add at least one procurement item.';
    ...
    if (!unit || (!COMMON_UNITS.has(unit) && !/^[a-z][a-z -]{1,29}$/i.test(unit))) errors[`item_${index}_unit`] = `Item ${index + 1}: enter a valid unit (e.g., pcs, sets, boxes, reams).`;
