...
BEGIN
  ...
  RETURN 'PR-' || to_char(current_date, 'YYYY') || '-' || lpad(next_no::text, 4, '0');
END;
$$;
...
REVOKE ALL ON FUNCTION public.assign_purchase_request_number() FROM PUBLIC;
REVOKE ALL ON SEQUENCE public.purchase_request_number_seq FROM PUBLIC;
