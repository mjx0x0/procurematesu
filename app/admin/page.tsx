...
const LABELS: Record<string, string> = { ...PROCUREMENT_STAGE_LABELS, draft: "Draft", rejected: "Rejected", cancelled: "Cancelled" };
const COLORS: Record<string, string> = {
  ...
  draft: "bg-gray-100 text-gray-600", rejected: "bg-red-100 text-red-700", cancelled: "bg-red-100 text-red-700",
};
...
export default function AdminDashboard() {
  ...
  const [rfqPrNo, setRfqPrNo] = useState<string | null>(null);
  ...
  useEffect(() => {
    const channel = supabase.channel("admin-live-procurement-updates")
      .on("postgres_changes", { event: "*", schema: "public", table: "purchase_requests" }, (payload) => {
        void loadData();
        if (selectedPRRef.current && payload.new && (payload.new as any).pr_no === selectedPRRef.current.pr_no) setSelectedPR(payload.new as PR);
      })
  ...
  const filtered = useMemo(() => prs.filter((p) => {
    ...
    return (!q || p.pr_no.toLowerCase().includes(q) || p.purpose?.toLowerCase().includes(q) || p.department?.toLowerCase().includes(q)) &&
  ...
  const stats = useMemo(() => ({
    total: prs.length,
    pending: prs.filter((p) => p.current_stage !== "completed" && !["rejected", "cancelled"].includes(p.current_stage)).length,
  ...
  const openDetails = async (pr: PR) => { setSelectedPR(pr); await loadHistory(pr.pr_no); setShowDetails(true); };
  const nextStage = (pr: PR) => { const i = STAGES.findIndex((s) => s.key === pr.current_stage); return i >= 0 && i < STAGES.length - 1 ? STAGES[i + 1] : null; };
  const openAction = (type: "complete" | "remark" | "reject", targetPR?: PR) => { if (targetPR) setSelectedPR(targetPR); setAction(type); setRemarks(""); };
  const openRfq = (pr: PR) => { setShowDetails(false); setAction(null); setRfqPrNo(pr.pr_no); };
  ...
  const submitAction = async () => {
    ...
    try {
      ...
      if (action === "complete") showFeedback("success", "Stage Completed Successfully", `${selectedPR.pr_no} has been advanced to Step ${nextInfo?.number || ""}: ${nextInfo?.label || LABELS[result.newStatus || ""]}.`);
      else if (action === "remark") showFeedback("success", "Remark Sent", `Your procurement remark for ${selectedPR.pr_no} has been recorded.`);
    ...
    <div className="min-h-screen bg-[#F9F7F4] text-gray-800 admin-dashboard-page">
      <nav className="bg-white/95 border-b border-stone-200 sticky top-0 z-40 shadow-[0_1px_12px_rgba(45,20,10,0.04)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <div className="bg-[#7C1D2E] p-2 rounded-xl text-[#D4A843] shrink-0"><FileText className="h-5 w-5" /></div>
            <div className="min-w-0"><b className="block text-base sm:text-lg text-[#5A1420] truncate">MSU GenSan Procurement Management System</b><span className="text-[10px] sm:text-xs text-stone-500 font-semibold tracking-wide">ADMIN PORTAL</span></div>
          </div>
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <span className="hidden lg:block text-xs text-stone-600"><User className="inline h-3.5 w-3.5 mr-1" />{user?.email}</span>
            <button onClick={logout} title="Logout" aria-label="Logout" className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg border border-stone-200 bg-white text-stone-600 hover:text-[#7C1D2E] hover:bg-red-50 text-xs font-bold transition-colors"><LogOut className="h-4 w-4" /><span>Logout</span></button>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-7">
        <div className="flex flex-col lg:flex-row justify-between gap-5 mb-7">
          <div><div className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-[#9A7B2F] font-extrabold mb-2"><Sparkles className="h-3.5 w-3.5" /> Procurement Operations</div><h1 className="text-3xl font-extrabold tracking-tight text-[#5A1420]">Admin Dashboard</h1><p className="text-stone-600 mt-1 text-sm">Manage Purchase Requests through the official 20-step PMO procurement process.</p></div>
          <div className="flex flex-wrap gap-2 items-start">
            <Link href="/admin/users" className="px-4 py-2.5 bg-white border border-stone-200 rounded-xl text-sm font-semibold text-[#7C1D2E] hover:bg-red-50 flex items-center gap-2"><Users className="h-4 w-4" /> User Approvals</Link>
            <Link href="/admin/inquiries" className="px-4 py-2.5 bg-white border border-stone-200 rounded-xl text-sm font-semibold text-[#7C1D2E] hover:bg-red-50 flex items-center gap-2"><MessageSquare className="h-4 w-4" /> Inquiries</Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-7">
          {[[stats.total, "Total PRs", FileCheck, "text-[#7C1D2E]"], [stats.pending, "In Progress", Clock, "text-amber-600"], [stats.inProgress, "Beyond Receipt", AlertCircle, "text-orange-600"], [stats.completed, "Completed", CheckCircle, "text-emerald-700"], [stats.rejected, "Rejected", XCircle, "text-red-600"]].map(([n, l, I, c]: any) => <div key={l} className="bg-white rounded-2xl p-4 border border-stone-200 shadow-[0_5px_20px_rgba(45,20,10,0.035)]"><div className="flex justify-between items-center"><div><div className={`text-2xl font-bold ${c}`}>{n}</div><div className="text-xs font-semibold text-stone-600 mt-0.5">{l}</div></div><I className={`h-5 w-5 ${c}`} /></div></div>)}
        </div>

        <div className="bg-white rounded-2xl border border-stone-200 p-4 mb-5 flex flex-col lg:flex-row gap-3 shadow-[0_5px_20px_rgba(45,20,10,0.03)]">
          <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-stone-400" /><input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search PR number, purpose, or department" className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-stone-200 bg-white text-gray-900 placeholder:text-stone-400 outline-none focus:ring-2 focus:ring-[#7C1D2E]/20 focus:border-[#7C1D2E]" /></div>
          <div className="flex flex-col sm:flex-row gap-3"><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2.5 rounded-xl border border-stone-200 bg-white text-gray-900"><option value="all">All Statuses</option>{STAGES.map((s) => <option key={s.key} value={s.key}>{s.number}. {s.shortLabel}</option>)}<option value="rejected">Rejected</option></select><select value={departmentFilter} onChange={(e) => setDepartmentFilter(e.target.value)} className="px-3 py-2.5 rounded-xl border border-stone-200 bg-white text-gray-900"><option value="all">All Departments</option>{departments.map((d) => <option key={d} value={d}>{d}</option>)}</select></div>
        </div>

        <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-[0_8px_30px_rgba(45,20,10,0.04)]">
          <div className="px-5 py-4 border-b border-stone-200 flex justify-between items-center"><div><h2 className="font-bold text-[#5A1420]">Purchase Requests</h2><p className="text-xs text-stone-500 mt-1">Workflow stages are enforced in order; administrators cannot skip steps.</p></div><span className="text-xs bg-stone-100 px-2.5 py-1 rounded-full font-bold text-stone-700">{filtered.length}</span></div>
          {filtered.length === 0 ? <div className="p-12 text-center text-stone-500">No purchase requests found.</div> : <div className="overflow-x-auto"><table className="w-full min-w-[1050px]"><thead className="bg-stone-50"><tr>{["PR #", "Purpose", "Department", "Amount", "Current Status", "Date", "Actions"].map((h) => <th key={h} className="px-5 py-3 text-left text-xs uppercase tracking-wide text-stone-500 font-semibold">{h}</th>)}</tr></thead><tbody className="divide-y divide-stone-100">
            {filtered.map((pr) => {
              const stepInfo = STAGES.find((s) => s.key === pr.current_stage); const nxt = nextStage(pr); const isRfqStage = pr.current_stage === "rfq_generation";
              return <tr key={pr.pr_no} className={`hover:bg-stone-50/70 transition-colors ${isRfqStage ? "bg-orange-50/25" : ""}`}>
                <td className="px-5 py-4 text-sm font-bold text-[#7C1D2E] whitespace-nowrap">{pr.pr_no}{isRfqStage && <span className="ml-2 inline-flex align-middle px-1.5 py-0.5 rounded-full bg-[#7C1D2E] text-white text-[9px] font-extrabold tracking-wide">RFQ</span>}</td>
                ...
                <td className="px-5 py-4"><span className={`px-2.5 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${COLORS[pr.current_stage] || "bg-gray-100 text-gray-600"}`}>{stepInfo?.number ? `Step ${stepInfo.number}: ` : ""}{LABELS[pr.current_stage] || pr.current_stage}</span></td>
                <td className="px-5 py-4 text-sm text-stone-500 whitespace-nowrap">{new Date(pr.created_at).toLocaleDateString()}</td>
                <td className="px-5 py-4"><div className="flex justify-end gap-1.5 items-center">
                  {isRfqStage && <button onClick={() => openRfq(pr)} title={`Generate RFQ for ${pr.pr_no}`} className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-[#7C1D2E] hover:bg-[#5A1420] text-white border border-[#7C1D2E] text-xs font-extrabold shadow-sm hover:shadow-md transition-all"><FileText className="h-3.5 w-3.5 text-[#D4A843]" /><span>Generate RFQ</span></button>}
                  ...
                  <button onClick={() => void openDetails(pr)} title="View / manage details" className="p-2 rounded-lg text-[#7C1D2E] hover:bg-red-50 transition-colors"><Eye className="h-4 w-4" /></button>
      ...
      {showDetails && selectedPR && <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] overflow-y-auto"><div className="sticky top-0 bg-white border-b border-stone-200 p-5 flex justify-between items-start z-10"><div><h3 className="text-xl font-bold text-[#5A1420]">PR {selectedPR.pr_no}</h3><p className="text-sm text-stone-500 mt-1">{selectedPR.department} · ₱{Number(selectedPR.total || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div><button onClick={() => setShowDetails(false)} className="p-2 text-stone-400 hover:text-stone-700 rounded-lg hover:bg-stone-100"><X className="h-5 w-5" /></button></div><div className="p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 p-4 bg-stone-50 rounded-xl border border-stone-200"><div><p className="text-xs uppercase tracking-wide text-stone-400 font-semibold">Current Stage</p><span className={`inline-block mt-1 px-3 py-1 rounded-full text-xs font-bold ${COLORS[selectedPR.current_stage] || "bg-gray-100"}`}>{selectedPR.current_stage === "rfq_generation" ? "Step 7: RFQ Generation" : LABELS[selectedPR.current_stage] || selectedPR.current_stage}</span><p className="text-xs text-stone-500 mt-1 font-medium">{STAGES.find((s) => s.key === selectedPR.current_stage)?.number ? `Step ${STAGES.find((s) => s.key === selectedPR.current_stage)?.number} of 20 in PMO Sequence` : "Workflow status"}</p></div><div className="flex gap-2 flex-wrap items-center">{selectedPR.current_stage === "rfq_generation" && <button onClick={() => openRfq(selectedPR)} className="px-3.5 py-2 bg-[#7C1D2E] text-white rounded-xl text-xs font-bold flex gap-1.5 items-center shadow-sm hover:bg-[#5A1420]"><FileText className="h-4 w-4 text-[#D4A843]" /> Generate RFQ</button>}{nextStage(selectedPR) && <button onClick={() => openAction("complete")} className="px-3.5 py-2 bg-white border border-[#D4A843] text-[#7C1D2E] rounded-xl text-xs font-bold flex gap-1.5 items-center hover:bg-amber-50"><Check className="h-4 w-4 text-[#D4A843]" /> Complete Next Stage</button>}{!["completed", "rejected", "cancelled"].includes(selectedPR.current_stage) && <><button onClick={() => openAction("remark")} className="px-3 py-2 bg-white border border-[#7C1D2E]/30 text-[#7C1D2E] rounded-xl text-xs font-bold flex gap-1 items-center hover:bg-red-50"><MessageSquare className="h-4 w-4" /> Remark</button><button onClick={() => openAction("reject")} className="px-3 py-2 bg-red-50 border border-red-200 text-red-700 rounded-xl text-xs font-bold flex gap-1 items-center hover:bg-red-100"><XCircle className="h-4 w-4" /> Reject</button></>}</div></div>
        {selectedPR.current_stage !== "rejected" && selectedPR.current_stage !== "cancelled" && <div className="bg-amber-50/60 border border-amber-200/70 rounded-xl p-4 mb-5"><p className="text-xs uppercase tracking-wide text-amber-800 font-bold mb-1">Current PMO Step</p><p className="text-sm font-bold text-stone-900">Step {STAGES.find((s) => s.key === selectedPR.current_stage)?.number}: {STAGES.find((s) => s.key === selectedPR.current_stage)?.label}</p><p className="text-xs text-stone-600 mt-1">{STAGES.find((s) => s.key === selectedPR.current_stage)?.description}</p></div>}
      ...
      {rfqPrNo && <RFQEditorModal prNo={rfqPrNo} onClose={() => setRfqPrNo(null)} onSaved={() => { void loadData(); }} />}
