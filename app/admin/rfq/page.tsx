  ...
  return (
    <div className="min-h-screen bg-[#F9F7F4] text-gray-800">
      <nav className="bg-white/95 border-b border-stone-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/admin" className="p-2 rounded-lg text-stone-500 hover:text-[#7C1D2E] hover:bg-stone-50" title="Back to Admin Dashboard">
              <ArrowLeft className="h-5 w-5" />
            </Link>
            <div className="bg-[#7C1D2E] p-2 rounded-xl text-[#D4A843]"><FileCheck2 className="h-5 w-5" /></div>
            <div>
              <b className="text-xl text-[#5A1420]">RFQ Workspace</b>
              <p className="text-[11px] text-stone-500">Official MSU-Gensan RFQ generation and printing</p>
            </div>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 py-7">
        <div className="mb-6">
          <h1 className="text-3xl font-extrabold text-[#5A1420]">Requests for Quotation</h1>
          <p className="text-stone-600 mt-1">Generate RFQs at Step 7, review them at Step 8, and print them at Step 9.</p>
        </div>

        <div className="bg-white rounded-xl border border-stone-200 p-4 mb-5 flex flex-col lg:flex-row gap-3">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-stone-400" />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search PR number, purpose, or department" className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-stone-200 bg-white text-gray-900 placeholder:text-stone-400 outline-none focus:ring-2 focus:ring-[#7C1D2E]/20 focus:border-[#7C1D2E]" />
          </div>
        ...
        <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
          {loading ? (
            <div className="p-12 flex justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#7C1D2E]" /></div>
          ) : filtered.length === 0 ? (
            <div className="p-12 text-center text-stone-500">No Purchase Requests are currently in the RFQ stages.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-stone-50">
                  <tr>
                    {['PR #', 'Purpose', 'Department', 'ABC', 'Current Stage', 'Action'].map((header) => <th key={header} className="px-5 py-3 text-left text-xs uppercase tracking-wide text-stone-500 font-semibold">{header}</th>)}
                  </tr>
                </thead>
                  ...
                  {filtered.map((pr) => (
                    <tr key={pr.pr_no} className="hover:bg-stone-50/70">
                      ...
                      <td className="px-5 py-4 text-sm">
                        <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-50 text-orange-700 border border-orange-100">{stageLabel(pr.current_stage)}</span>
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap">
                        {pr.current_stage === "rfq_generation" ? <RFQActionButton prNo={pr.pr_no} mode="generate" /> : pr.current_stage === "rfq_printing" ? <RFQActionButton prNo={pr.pr_no} mode="print" /> : <span className="text-xs text-stone-400">Awaiting Step 9</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
