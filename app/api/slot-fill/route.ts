...
const MAX_MESSAGE_LENGTH = 6000; const MAX_ITEMS = 50; const MAX_QTY = 1000000; const MAX_UNIT_COST = 1000000000; const MAX_TOTAL = 100000000000; const JSON_HEADERS = { 'Cache-Control': 'no-store' };
...
function fallbackExtraction(message: string) {
  ...
  const qty = Math.min(Math.max(qtyMatch ? parseInt(qtyMatch[1], 10) : 1, 1), MAX_QTY); const unit = (qtyMatch && qtyMatch[2]) || 'units'; const desc = ((qtyMatch && qtyMatch[3]?.trim()) || message.slice(0, 60)).slice(0, 500); const costMatch = message.match(/(?:cost|price|budget|around|₱|php)?\s*[:=]?\s*(\d+[\d,]*)/i); const unitCost = Math.min(Math.max(costMatch ? parseInt(costMatch[1].replace(/,/g, ''), 10) : 5000, 0), MAX_UNIT_COST);
  return { department: 'Mindanao State University - General Santos', purpose: message.slice(0, 1000), items: [{ item_description: desc, quantity: qty, unit, unit_cost: unitCost, total_cost: qty * unitCost }], total_amount: qty * unitCost };
}

function normalizeExtraction(value: unknown) {
  if (!value || typeof value !== 'object') return null; const input = value as Record<string, unknown>; const rawItems = Array.isArray(input.items) ? input.items.slice(0, MAX_ITEMS) : [];
  const items = rawItems.map((item) => { const row = item && typeof item === 'object' ? item as Record<string, unknown> : {}; const quantity = Math.min(Math.max(Number(row.quantity) || 1, 1), MAX_QTY); const unitCost = Math.min(Math.max(Number(row.unit_cost) || 0, 0), MAX_UNIT_COST); const description = typeof row.item_description === 'string' ? row.item_description.trim().slice(0, 500) : ''; const unit = typeof row.unit === 'string' ? row.unit.trim().slice(0, 50) : 'units'; return { item_description: description, quantity: Math.floor(quantity), unit, unit_cost: unitCost, total_cost: Math.min(quantity * unitCost, MAX_TOTAL) }; }).filter((item) => item.item_description.length > 0);
  const totalAmount = Math.min(items.reduce((sum, item) => sum + item.total_cost, 0), MAX_TOTAL); return { department: typeof input.department === 'string' ? input.department.trim().slice(0, 300) : null, purpose: typeof input.purpose === 'string' ? input.purpose.trim().slice(0, 1000) : null, items, total_amount: totalAmount };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (origin && host) { try { if (new URL(origin).host !== host) return NextResponse.json({ error: 'Invalid request origin.' }, { status: 403, headers: JSON_HEADERS }); } catch { return NextResponse.json({ error: 'Invalid request origin.' }, { status: 403, headers: JSON_HEADERS }); } }
    const body = await req.json(); const message = typeof body?.message === 'string' ? body.message.trim() : '';
    if (!message) return NextResponse.json({ error: 'Message is required.' }, { status: 400, headers: JSON_HEADERS });
    if (message.length > MAX_MESSAGE_LENGTH) return NextResponse.json({ error: `Message must not exceed ${MAX_MESSAGE_LENGTH} characters.` }, { status: 413, headers: JSON_HEADERS });
    const validationError = validateDraftInput(message);
