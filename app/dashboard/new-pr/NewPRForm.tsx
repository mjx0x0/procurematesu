...
export default function NewPRForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    department: "",
    section: "",
    // SAI and ALOBS will be auto-generated by the database, so we don't need them in state
    total_amount: 0,
    ...
    if (itemsParam) {
      try {
        ...
        if (Array.isArray(parsedItems) && parsedItems.length > 0) {
          ...
          setItems(newItems);
          const total = newItems.reduce((sum, item) => sum + (item.total_cost || 0), 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure actual name is provided, not an email address
      if (!userName.trim()) {
        setError("Please enter your actual full name in the 'Requested By' field.");
        setSubmitting(false);
        return;
      }

      if (userName.includes("@")) {
        setError("Please enter your actual full name (e.g., Prof. Juan Dela Cruz), not your email address.");
        setSubmitting(false);
        return;
      }

      // Update user metadata in background to persist real name for subsequent PRs
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl shadow-sm border border-stone-200/90 p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-200">
              <div>
                <label className="block text-sm font-semibold text-stone-800 mb-1">
                  Requested By (Actual Full Name) <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  required
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 animate-fade-in-up">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Describe what items or supplies your department needs, and AI will automatically structure the purchase request items and specifications.
            </p>
