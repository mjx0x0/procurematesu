...
export default function ProcurementRequestForm() {
  ...
  useEffect(() => { (async () => { const { data: { user } } = await supabase.auth.getUser(); if (!user) { router.push("/auth/login"); return; } setUserId(user.id); const name = user.user_metadata?.full_name || ""; setUserName(name && !name.includes("@") ? name : ""); setLoading(false); })(); }, [router]);
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5]">
      <nav className="bg-white/90 backdrop-blur-md border-b border-stone-200 px-4 py-3 sticky top-0 z-50"><div className="max-w-7xl mx-auto flex justify-between items-center"><Link href="/dashboard" className="flex items-center gap-3 text-[#4D0C0D]"><ArrowLeft className="h-5 w-5 text-stone-600" /><span className="bg-[#7A1315] p-2 rounded-xl text-amber-200 border border-amber-400/30"><FileText className="h-5 w-5" /></span><span className="font-bold text-xl">New Purchase Request</span></Link><button onClick={() => setShowAi(true)} className="bg-gradient-to-r from-[#B88E13] to-[#D4AF37] text-[#4D0C0D] px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 border border-amber-500/40"><Sparkles className="h-4 w-4" />Draft with AI</button></div></nav>
      <main className="max-w-5xl mx-auto px-4 py-8"><div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6 md:p-8"><form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">PR Number</label><input value="Auto-generated" disabled className="w-full px-4 py-2 border border-stone-200 rounded-lg bg-stone-50 text-stone-500 font-medium" /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Date</label><input type="date" value={new Date().toISOString().split("T")[0]} disabled className="w-full px-4 py-2 border border-stone-200 rounded-lg bg-stone-50 text-stone-500 font-medium" /></div></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-700 mb-1">Department <span className="text-red-500">*</span></label><input type="text" value={form.department} onChange={e => setForm({ ...form, department: e.target.value })} placeholder="Enter your department (e.g., College of Education)" required className="w-full px-4 py-2.5 border border-stone-300 rounded-lg bg-white text-gray-900 placeholder:text-stone-400 focus:ring-2 focus:ring-[#7A1315]/20 focus:border-[#7A1315] outline-none" /><p className="text-xs text-stone-500 mt-1">Enter the department/unit you belong to. There are no preset choices.</p></div><div><label className="block text-sm font-medium text-gray-700 mb-1">Section <span className="text-stone-400 font-normal">(Optional)</span></label><input type="text" value={form.section} onChange={e => setForm({ ...form, section: e.target.value })} placeholder="Enter section if applicable" className="w-full px-4 py-2.5 border border-stone-300 rounded-lg bg-white text-gray-900 placeholder:text-stone-400 focus:ring-2 focus:ring-[#7A1315]/20 focus:border-[#7A1315] outline-none" /><p className="text-xs text-stone-500 mt-1">You may leave this blank.</p></div></div>
        <div><label className="block text-sm font-medium text-gray-700 mb-1">SAI No.</label><input value="Auto-generated" disabled className="w-full px-4 py-2 border border-stone-200 rounded-lg bg-stone-50 text-stone-500 font-medium" /></div><div><label className="block text-sm font-medium text-gray-700 mb-1">ALOBs No.</label><input value="Auto-generated" disabled className="w-full px-4 py-2 border border-stone-200 rounded-lg bg-stone-50 text-stone-500 font-medium" /></div>
