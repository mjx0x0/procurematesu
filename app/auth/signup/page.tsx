          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a secure password" autoComplete="new-password" className="w-full pl-9 pr-11 py-2.5 text-sm border border-stone-300 rounded-xl bg-white text-gray-900 placeholder:text-stone-400 focus:ring-2 focus:ring-[#7A1315]/20 focus:border-[#7A1315] outline-none transition-all" required />
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600" aria-label={showConfirm ? "Hide password" : "Show password"}>{showConfirm ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
              </div>
            </div>
            ...
            <p className="text-center text-xs text-stone-600 pt-1">
              Already have an account? <Link href="/auth/login" className="font-semibold text-[#7A1315] hover:text-[#4D0C0D]">Sign in</Link>
            </p>
          </form>
        </div>
      </div>
