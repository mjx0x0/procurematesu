...
export function ChatMessageContent({content,isUser=false}:{content:string;isUser?:boolean}){
 ...
 return <div className="space-y-2 text-sm leading-relaxed text-gray-800">{clean.map((line,i)=>{
  ...
  const heading=t.match(/^#{1,6}\s+(.+)$/); if(heading)return <div key={i} className="pt-1 text-[13px] font-bold text-[#4D002C]">{inline(heading[1])}</div>;
  const link=line.match(/\[([^\]]+)\]\(([^)]+)\)/); if(link){const isNew=link[2].includes("new-pr"),isPrint=link[2].includes("pr-print");return <div key={i} className="my-2"><Link href={link[2]} target={isPrint?"_blank":"_self"} rel="noopener noreferrer" className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold text-white shadow-sm ${isNew?"bg-[#7B0046] hover:bg-[#610037]":isPrint?"bg-[#087449] hover:bg-[#06633e]":"bg-gray-800 hover:bg-gray-900"}`}>{isNew?<FileText className="h-4 w-4"/>:isPrint?<Printer className="h-4 w-4"/>:<ExternalLink className="h-3.5 w-3.5"/>}<span>{link[1]}</span><ArrowRight className="h-3.5 w-3.5 opacity-70"/></Link></div>}
  const bullet=/^[•\-]\s+(.+)$/.exec(t); if(bullet)return <div key={i} className="flex items-start gap-2 pl-1"><span className="mt-1 text-[#F5AB26]">•</span><div className="flex-1">{inline(bullet[1])}</div></div>;
  const numbered=/^(\d+\.)\s+(.+)$/.exec(t); if(numbered)return <div key={i} className="flex items-start gap-2 pl-1"><span className="min-w-5 text-xs font-semibold text-[#7B0046]">{numbered[1]}</span><div className="flex-1">{inline(numbered[2])}</div></div>;
  if(/^https?:\/\//.test(t))return <Link key={i} href={t} target="_blank" className="text-[#7B0046] underline">{t}</Link>;
