...
export function Chatbot() {
  ...
  const sendMessage = async (textToSend: string) => {
    ...
    try {
      // The current chat API accepts a single message. We include the recent
      // conversation as temporary context so the model can remember prior turns;
      // the database still stores only the clean user message below.
      ...
      // Replace the context-expanded user row written by /api/chat with the
      // clean message shown in the UI. This keeps persistent history readable.
      const { data: recentUser } = await supabase
        ...
        .maybeSingle();
      if (recentUser?.id) {
        await supabase.from("chat_messages").update({ content: text }).eq("id", recentUser.id);
      }
      ...
      setMessages((prev) => [...prev, {
        id: `error-${Date.now()}`,
        role: "assistant",
        content: "⚠️ We encountered a temporary connection issue. Your conversation history is preserved; please retry.",
        timestamp: new Date(),
      }]);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      {!isOpen && (
        <button onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 bg-gradient-to-r from-[#7A1315] to-[#4D0C0D] hover:from-[#630E10] hover:to-[#7A1315] text-white p-4 rounded-full shadow-xl z-50 flex items-center gap-2.5 border border-amber-400/30" aria-label="Open AI Procurement Assistant">
          <Bot className="h-6 w-6 text-amber-300" />
          <span className="hidden sm:inline font-semibold text-sm pr-1">AI Procurement Assistant</span>
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
        </button>
      )}

      {isOpen && (
        <div className={`fixed bottom-6 right-6 bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-red-950/15 z-50 flex flex-col ${isMinimized ? "w-80 h-14" : "w-[94vw] sm:w-[760px] h-[650px] max-h-[88vh]"}`}>
          <div className="flex justify-between items-center px-4 py-3 border-b border-amber-400/20 bg-gradient-to-r from-[#4D0C0D] via-[#7A1315] to-[#630E10] text-white rounded-t-2xl">
            <div className="flex items-center gap-2.5">
              <div className="bg-[#7A1315] p-1.5 rounded-lg border border-amber-400/40"><Bot className="h-4 w-4 text-amber-300" /></div>
              <div><div className="flex items-center gap-1.5"><span className="font-bold text-sm">AI Procurement Assistant</span><span className="text-[10px] bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 px-1.5 py-0.2 rounded-full">Online</span></div><p className="text-[10px] text-amber-100/75">Mindanao State University - General Santos</p></div>
            </div>
            <div className="flex items-center gap-1">
              {!isMinimized && <button onClick={() => setHistoryOpen(!historyOpen)} className="p-1.5 hover:bg-white/10 rounded-lg" title="Chat history"><MessageSquarePlus className="h-4 w-4 text-amber-100" /></button>}
              <button onClick={() => setIsMinimized(!isMinimized)} className="p-1.5 hover:bg-white/10 rounded-lg" title={isMinimized ? "Expand" : "Minimize"}>{isMinimized ? <Maximize2 className="h-4 w-4" /> : <Minimize2 className="h-4 w-4" />}</button>
              <button onClick={() => setIsOpen(false)} className="p-1.5 hover:bg-white/10 rounded-lg" title="Close"><X className="h-4 w-4" /></button>
            </div>
          </div>
          ...
          {!isMinimized && (
            <div className="flex flex-1 min-h-0 relative">
              {historyOpen && (
                <aside className="w-64 shrink-0 border-r border-gray-200 bg-[#FAF8F5] flex flex-col">
                  <div className="p-3 border-b border-gray-200 flex items-center justify-between"><span className="font-bold text-xs text-gray-700">Chat history</span><button onClick={() => setHistoryOpen(false)}><ChevronLeft className="w-4 h-4 text-gray-500" /></button></div>
                  <button onClick={startNewChat} className="m-3 px-3 py-2 rounded-xl bg-[#7A1315] text-white text-xs font-semibold flex items-center justify-center gap-2"><MessageSquarePlus className="w-3.5 h-3.5" /> New chat</button>
                  <div className="overflow-y-auto px-2 pb-3 space-y-1">
                    {sessions.map((s) => <button key={s.id} onClick={() => selectSession(s.id)} className={`w-full text-left px-3 py-2.5 rounded-lg text-xs transition-colors ${s.id === sessionId ? "bg-red-100 text-[#7A1315]" : "hover:bg-white text-gray-600"}`}><div className="font-medium truncate">{s.title || "New conversation"}</div><div className="text-[10px] text-gray-400 mt-0.5">{new Date(s.updated_at || s.created_at).toLocaleDateString()}</div></button>)}
                    {!sessions.length && <p className="text-[11px] text-gray-400 text-center px-3 py-6">No previous conversations.</p>}
                  </div>
                </aside>
              )}
