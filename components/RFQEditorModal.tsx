...
function Input({ value, onChange, type = "text" }: { value: string | number; onChange: (value: string) => void; type?: string }) {
  return <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-md border border-stone-300 px-2 py-1.5 text-sm text-gray-900 bg-white focus:outline-none focus:border-[#7C1D2E]" />;
}
...
export default function RFQEditorModal({ prNo, onClose, onSaved }: { prNo: string; onClose: () => void; onSaved?: () => void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      } catch (e: any) { if (active) setError(e?.message || "Unable to load the RFQ."); }
  ...
  const setField = (key: keyof RFQFormDraft, value: string) => { setForm((current) => current ? { ...current, [key]: value } : current); setSaved(false); };
  ...
  const addItem = () => setForm((current) => current ? { ...current, items: [...current.items, { item: current.items.length + 1, quantity: 0, abc: 0, technical_specifications: "", supplier_unit: "", supplier_unit_price: "", supplier_total_amount: "" }] } : current);
  ...
  const save = async (printAfter = false) => {
    ...
    try {
      const response = await fetch("/api/admin/rfq", { method: "PATCH", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prNo, formData: form }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to save RFQ corrections.");
      setForm(data.formData as RFQFormDraft); setSaved(true); onSaved?.();
  ...
  const inputClass = "w-full rounded-md border border-stone-300 px-2 py-1.5 text-sm text-gray-900 bg-white focus:outline-none focus:border-[#7C1D2E]";
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6">
      <div className="w-full max-w-7xl max-h-[96vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <header className="px-5 py-4 border-b border-stone-200 flex items-center justify-between shrink-0"><div><h2 className="text-xl font-extrabold text-[#5A1420]">RFQ Form — {prNo}</h2><p className="text-xs text-stone-500">Automatically filled from the Purchase Request. Correct any errors before saving or printing.</p></div><div className="flex items-center gap-2"><span className={`px-3 py-1.5 rounded-full text-xs font-bold ${lessThan50k ? "bg-amber-50 text-amber-700" : "bg-purple-50 text-purple-700"}`}>{lessThan50k ? "RFQ LESS THAN 50 K" : "RFQ MORE THAN 50K"}</span><button onClick={onClose} className="p-2 rounded-lg hover:bg-stone-100 text-stone-500"><X className="h-5 w-5" /></button></div></header>
        <main className="overflow-y-auto bg-stone-100 p-4 sm:p-6">
          <section id="official-rfq-print" className="mx-auto max-w-6xl bg-white text-black border border-stone-300 shadow-sm p-6 print:border-0 print:shadow-none">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_310px] border border-black mt-2"><div className="flex items-center justify-center min-h-[70px]"><div className="text-center font-extrabold text-xl">REQUEST FOR QUOTATION</div></div><div className="border-t lg:border-t-0 lg:border-l border-black"><label className="grid grid-cols-[105px_1fr] border-b border-black text-xs"><span className="p-2">Reference Nos.</span><Input value={form.reference_no} onChange={v => setField("reference_no", v)} /></label><label className="grid grid-cols-[105px_1fr] border-b border-black text-xs"><span className="p-2">Project Name:</span><Input value={form.project_name} onChange={v => setField("project_name", v)} /></label><label className="grid grid-cols-[105px_1fr] text-xs"><span className="p-2">Location:</span><Input value={form.location} onChange={v => setField("location", v)} /></label></div></div>
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_310px] border-x border-b border-black"><div className="p-3 space-y-1 text-sm"><Input value={form.company_name} onChange={v => setField("company_name", v)} /><div className="text-xs">Company Name</div><Input value={form.address} onChange={v => setField("address", v)} /><div className="text-xs">Address</div></div><div className="border-t lg:border-t-0 lg:border-l border-black p-3 space-y-3 text-xs"><label className="flex items-center gap-2"><span className="w-24">Date :</span><Input type="date" value={form.rfq_date} onChange={v => setField("rfq_date", v)} /></label><label className="flex items-center gap-2"><span className="w-24">Quotation No. :</span><Input value={form.quotation_no} onChange={v => setField("quotation_no", v)} /></label></div></div>
            ...
            <div className="border-x border-b border-black overflow-x-auto"><table className="w-full border-collapse text-xs min-w-[900px]"><thead><tr><th rowSpan={2} className="border border-black p-2">Item</th><th rowSpan={2} className="border border-black p-2">QTY</th><th rowSpan={2} className="border border-black p-2">ABC</th><th rowSpan={2} className="border border-black p-2">Technical Specifications</th><th colSpan={2} className="border border-black p-2">Unit Price</th><th rowSpan={2} className="border border-black p-2">Total Amount</th><th className="print:hidden border-0" /></tr><tr><th className="border border-black p-2">UNIT</th><th className="border border-black p-2">UNIT PRICE</th></tr></thead><tbody>{form.items.map((item, index) => <tr key={index}><td className="border border-black p-1 text-center">{item.item}</td><td className="border border-black p-1"><Input type="number" value={item.quantity} onChange={v => setItem(index, "quantity", Number(v))} /></td><td className="border border-black p-1"><Input type="number" value={item.abc} onChange={v => setItem(index, "abc", Number(v))} /></td><td className="border border-black p-1"><textarea rows={2} value={item.technical_specifications} onChange={e => setItem(index, "technical_specifications", e.target.value)} className={inputClass} /></td><td className="border border-black p-1"><Input value={item.supplier_unit} onChange={v => setItem(index, "supplier_unit", v)} /></td><td className="border border-black p-1"><Input value={item.supplier_unit_price} onChange={v => setItem(index, "supplier_unit_price", v)} /></td><td className="border border-black p-1"><Input value={item.supplier_total_amount} onChange={v => setItem(index, "supplier_total_amount", v)} /></td><td className="print:hidden p-1"><button type="button" onClick={() => removeItem(index)} className="p-1 text-red-500 hover:bg-red-50 rounded"><Trash2 className="h-4 w-4" /></button></td></tr>)}<tr><td colSpan={4} className="border border-black p-2 text-center font-bold">***NOTHING FOLLOWS***</td><td className="border border-black" /><td className="border border-black" /><td className="border border-black" /><td className="print:hidden" /></tr><tr><td colSpan={4} className="border border-black p-2"><Input value={form.purpose} onChange={v => setField("purpose", v)} /></td><td colSpan={3} className="border border-black p-2"><Input value={form.office} onChange={v => setField("office", v)} /></td><td className="print:hidden" /></tr><tr><td colSpan={4} className="border border-black p-2 text-center font-bold">TOTAL ABC</td><td colSpan={3} className="border border-black p-2 text-right font-bold">₱{money(total)}</td><td className="print:hidden" /></tr><tr><td colSpan={2} className="border border-black p-2 text-center font-bold">Instructions:</td><td colSpan={5} className="border border-black p-2 italic"><Input value={form.instructions} onChange={v => setField("instructions", v)} /></td><td className="print:hidden" /></tr></tbody></table></div>
            <div className="text-right text-xs mt-1">(Please provide complete information below)</div><div className="ml-auto w-full sm:w-[310px] mt-3 space-y-2 text-xs"><label className="flex items-center gap-1">Delivery Period :<Input value={form.delivery_period} onChange={v => setField("delivery_period", v)} /></label><label className="flex items-center gap-1">Warranty:<Input value={form.warranty} onChange={v => setField("warranty", v)} /></label><label className="flex items-center gap-1">Price Validity :<Input value={form.price_validity} onChange={v => setField("price_validity", v)} /></label></div>
            <p className="text-xs mt-8">Unit Purchase/Job Order or a Contract is prepared and executed, this Quotation/Proposal shall be binding upon us. We understand that you are not bound to accept the lowest or any Proposal you may receive.</p><p className="text-xs mt-2">After having carefully read and accepted your General Conditions, I/We quote you on the item at prices noted above.</p><div className="grid grid-cols-2 gap-12 mt-10 text-xs"><div className="text-center"><Input value={form.canvasser_name} onChange={v => setField("canvasser_name", v)} /><div>Signature over printed name of canvasser</div></div><div><Input value={form.bidder_name} onChange={v => setField("bidder_name", v)} /><div>Signature over printed name of bidder</div><div className="mt-1">Tel. No. / Cellphone No.: <Input value={form.bidder_contact} onChange={v => setField("bidder_contact", v)} /></div><div className="mt-1">E-mail address: <Input value={form.bidder_email} onChange={v => setField("bidder_email", v)} /></div></div></div>
          </section>
        </main>
