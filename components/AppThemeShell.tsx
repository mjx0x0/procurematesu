...
export default function AppThemeShell({children}:{children:React.ReactNode}){
 ...
 return <div className={`app-theme ${isAdmin?"admin-shell":"portal-shell"}`}><SystemBranding/>{isAdmin&&<AppSidebar mode="admin"/>}{isAdmin&&pathname==="/admin"&&<RFQStagePopup/>}{children}</div>;
}
